...
void loop() {
  if (mySerial.available()) {   //read the two 
    ...
    if( command == 's'){
      ...
      my_msg.arg1 = mySerial.read()*2 - 200;
      my_msg.arg2 = mySerial.read()*2 - 200;
      pub.publish(&my_msg);
    }else if(command == 'l'){
      if(mySerial.read() == 'o'){
      ...
      my_msg.command = "set_cam";
      my_msg.arg1 = map(mySerial.read(),0,250,10,170);
      my_msg.arg2 = map(mySerial.read(),0,250,10,170);;
      pub.publish(&my_msg);
    }
